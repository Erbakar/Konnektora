...
export class TicketsService {
  ...
  async createType(eventId: string, input: ManageTicketTypeDto, actor: User) { await this.ensureManager(eventId, actor.id); const type = await this.prisma.eventTicketType.create({ data: { eventId, ...this.typeData(input) } }); return this.presentType(type); }
  async updateType(eventId: string, id: string, input: ManageTicketTypeDto, actor: User) { await this.ensureManager(eventId, actor.id); const existing = await this.prisma.eventTicketType.findFirst({ where: { id, eventId } }); if (!existing) throw new NotFoundException("Bilet türü bulunamadı."); if (input.capacity < existing.soldCount) throw new BadRequestException("Kapasite satılmış bilet sayısından düşük olamaz."); const type = await this.prisma.eventTicketType.update({ where: { id }, data: this.typeData(input) }); return this.presentType(type); }

  async purchase(ticketTypeId: string, quantity: number, buyer: User) {
    const type = await this.prisma.eventTicketType.findUnique({ where: { id: ticketTypeId }, include: { event: { select: { id: true, title: true, slug: true, status: true, createdById: true } } } });
    ...
    const order = await this.prisma.$transaction(async (tx) => {
      ...
      const created = await tx.eventTicketOrder.create({ data: { eventId: type.eventId, ticketTypeId: type.id, buyerId: buyer.id, paymentId: payment?.id, quantity, unitPrice: type.price, totalAmount: total, currency: type.currency, status: TicketOrderStatus.paid, purchasedAt: now, tickets: { create: qrTokens.map((token) => ({ eventId: type.eventId, ticketTypeId: type.id, ownerId: buyer.id, qrToken: token, qrTokenHash: this.hashToken(token) })) } }, include: { tickets: true } });
  ...
  async mine(userId: string) {
    ...
    return orders.map((order) => ({ id: order.id, status: order.status, quantity: order.tickets.length, unitPrice: Number(order.unitPrice), totalAmount: Number(order.totalAmount), currency: order.currency, purchasedAt: order.purchasedAt, event: { id: order.event.id, title: order.event.title, slug: order.event.slug, startsAt: order.event.startsAt, endsAt: order.event.endsAt, city: order.event.city, country: order.event.country, coverImageUrl: order.event.coverImageUrl }, ticketType: this.presentType(order.ticketType), tickets: order.tickets.map((ticket) => ({ id: ticket.id, status: ticket.status, createdAt: ticket.createdAt, usedAt: ticket.usedAt, qrPayload: `konnektora-ticket:${ticket.id}:${ticket.qrToken}` })) }));
  }
