...
const memberSelect = { id: true, name: true, username: true, city: true, country: true, followerCount: true } as const;
...
export class IdentityService {
  ...
  async onboardingStatus(userId: string) {
    const [user, photoCount, interestCount, followingCount] = await Promise.all([
      this.prisma.user.findUnique({ where: { id: userId }, select: { phoneVerified: true, username: true, country: true, birthDate: true, onboardingCompletedAt: true } }),
      this.prisma.mediaFile.count({ where: { contentType: "user", contentId: userId, status: "active", type: "image" } }),
  ...
  async completeOnboarding(userId: string) {
    ...
    const requiredMissing = status.steps.slice(0, 4).filter((step) => !step.completed);
    if (requiredMissing.length) throw new BadRequestException("Onboarding tamamlanmadan önce zorunlu adımları bitirin.");
  ...
  async memberPass(userId: string) {
    const user = await this.prisma.user.findUnique({ where: { id: userId }, select: { ...memberSelect, memberPassVersion: true, status: true } });
  ...
  async rotateMemberPass(userId: string) {
    await this.prisma.user.update({ where: { id: userId }, data: { memberPassVersion: { increment: 1 } } });
    return this.memberPass(userId);
  }

  async scan(scannerId: string, input: ScanMemberDto) {
    const token = this.extractToken(input.payload);
    let claim: { sub?: string; purpose?: string; version?: number };
    try { claim = await this.jwt.verifyAsync(token); } catch { throw new BadRequestException("Üye kartı geçersiz veya süresi dolmuş."); }
    if (!claim.sub || claim.purpose !== "member-pass" || claim.sub === scannerId) throw new BadRequestException("Üye kartı geçersiz.");
    const [member, block, existing] = await Promise.all([
      this.prisma.user.findUnique({ where: { id: claim.sub }, select: { ...memberSelect, memberPassVersion: true, status: true } }),
      this.prisma.userBlock.findFirst({ where: { targetType: "user", OR: [{ userId: scannerId, targetId: claim.sub }, { userId: claim.sub, targetId: scannerId }] }, select: { userId: true } }),
      this.prisma.userFollow.findUnique({ where: { followerId_followingId: { followerId: scannerId, followingId: claim.sub } } })
    ]);
    if (!member || member.status !== UserStatus.active || member.memberPassVersion !== claim.version) throw new BadRequestException("Üye kartı artık geçerli değil.");
    if (block) throw new ForbiddenException("Engellenen kullanıcı kartı taranamaz.");
    const scan = await this.prisma.$transaction(async (tx) => {
      if (!existing) {
        await tx.userFollow.create({ data: { followerId: scannerId, followingId: member.id } });
        await tx.user.update({ where: { id: scannerId }, data: { followingCount: { increment: 1 } } });
        await tx.user.update({ where: { id: member.id }, data: { followerCount: { increment: 1 } } });
      }
  ...
  async scanHistory(userId: string) {
    const scans = await this.prisma.memberScan.findMany({ where: { scannerId: userId }, orderBy: { createdAt: "desc" }, take: 100, include: { member: { select: memberSelect } } });
    return scans.map((scan) => ({ id: scan.id, method: scan.method, createdAt: scan.createdAt, member: scan.member, following: true }));
  }
  ...
  private extractToken(payload: string) {
    ...
    if (!token) throw new BadRequestException("Üye kartı token bilgisi içermiyor.");
    return token;
  }
}
