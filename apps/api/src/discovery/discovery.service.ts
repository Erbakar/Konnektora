...
export class DiscoveryService {
  ...
  private userItem(user: { id: string; name: string; username: string | null; city: string | null; country: string | null; followerCount: number }) { return { kind: "user" as const, id: user.id, title: user.name, subtitle: user.username ? `@${user.username}` : null, href: `/messages?peer=${user.id}`, imageUrl: null, meta: `${user.followerCount} takipçi${user.city || user.country ? ` · ${user.city ?? user.country}` : ""}` }; }
