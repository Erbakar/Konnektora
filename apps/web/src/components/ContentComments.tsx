      ...
      <div className="admin-list">
        {filteredComments.map((comment) => (
          <article className="admin-list-row" key={comment.id}>
            {comment.author?.avatarUrl ? <img className="comment-author-avatar" src={comment.author.avatarUrl} alt=""/> : <span className="comment-author-avatar comment-author-fallback">{comment.author?.name?.[0] ?? "?"}</span>}
            <div>
              <strong>
                {comment.author ? (
...
function CommentActions({ comment, currentUserId, following, targetType, canManage, onChanged }: { comment: ContentThreadComment; currentUserId?: string; following: boolean; targetType: "event" | "place" | "tag_comment"; canManage: boolean; onChanged: () => void }) {
  ...
  const report = useMutation({ mutationFn: () => createContentReport({ targetType: "comment_reply", targetId: comment.id, reason: "Uygunsuz yorum" }) });
  ...
  return <div className="comment-actions">
    ...
    {comment.authorId === currentUserId ? <button onClick={() => { const body = window.prompt("Yorumu düzenle", comment.body); if (body?.trim() && body.trim() !== comment.body) edit.mutate(body.trim()); }} type="button"><Edit3 size={14}/>Düzenle</button> : <button disabled={report.isPending} onClick={() => report.mutate()} type="button">Rapor et</button>}
