...
export function AnnouncementPopup({ announcements }: { announcements: Announcement[] }) {
  ...
  const title = language === "en" ? active.titleEn?.trim() || active.title : active.title;
  const body = language === "en" ? active.bodyEn?.trim() || active.body : active.body;
