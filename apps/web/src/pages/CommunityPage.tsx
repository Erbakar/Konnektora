...
import { followUser, getUserSession, inviteEventParticipant, listEventParticipants, listFollowing, listMemberSuggestions, listMyEvents, unfollowUser } from "../lib/api";
import { UserIdentityLink, userProfilePath } from "../components/UserIdentityLink";

type DirectoryTab = "popular" | "following" | "guests" | "mutual";
...
export function CommunityPage() {
  ...
  const [tab, setTab] = useState<DirectoryTab>(requestedTab === "following" || requestedTab === "guests" || requestedTab === "mutual" ? requestedTab : "popular");
  ...
  const members: GuestMember[] = tab === "following"
    ? following.data ?? []
    : tab === "guests"
      ? guestMembers
      : [...(suggestions.data ?? [])]
  ...
  return <div className="page community-page">
    ...
    <div className="feed-tabs" role="tablist">
      ...
      <button className={tab === "guests" ? "active" : ""} onClick={() => setTab("guests")}><CalendarCheck size={17}/> Guest Lists</button>
      <button className={tab === "mutual" ? "active" : ""} onClick={() => setTab("mutual")}><Sparkles size={17}/> Mutual hits</button>
    </div>
    {pending ? <div className="feed-state">Kişiler yükleniyor…</div> : members.length ? <section className="community-grid">{members.map((member) => <article className="community-card" key={member.id}>
      ...
      <div className="community-card-body"><Link to={userProfilePath(member)}><strong>{member.name}</strong></Link>{member.username ? <span>@{member.username}</span> : null}{member.birthDate ? <span>{member.gender === "male" ? "He" : member.gender === "female" ? "She" : "They"} is {memberAge(member.birthDate)} y.o.</span> : null}<span><MapPin size={14}/>{member.city || member.country ? `Based in ${member.city || member.country}.` : "Konum belirtilmedi"}</span><small>{member.followerCount} takipçi · {member.commonTagCount} ortak ilgi alanı</small>{tab === "guests" && member.guestEvents ? <small>{member.guestEvents}</small> : null}</div>
      <div className="community-card-actions"><button className={member.following ? "secondary-action" : "primary-action"} disabled={toggle.isPending} onClick={() => toggle.mutate(member)}>{member.following ? "Following" : "Follow"}</button><button className="secondary-action" onClick={() => setGuestTarget(member)}><UserPlus size={16}/> Guest list</button><details className="action-menu"><summary aria-label="Kullanıcı aksiyonları"><MoreVertical size={18}/></summary><div><Link to={`/messages?peer=${member.id}`}><Mail size={17}/> Mesaj gönder</Link></div></details></div>
