...
export function SearchPage() {
  ...
  return <section className="page search-page">
    <header><span className="eyebrow">Konnektora Discovery</span><h1>Her şeyi ara</h1><p>Üyeleri, ilgi alanlarını, etkinlikleri ve mekânları tek yerden bul.</p></header>
    <form className="global-search" onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const value = String(new FormData(event.currentTarget).get("q") ?? "").trim(); if (value.length >= 2) setParams({ q: value }); }}>
      <Search size={22} /><input aria-label="Arama" defaultValue={query} name="q" placeholder="Search anything" minLength={2} required /><button className="primary-action" type="submit">Ara</button>
    </form>
