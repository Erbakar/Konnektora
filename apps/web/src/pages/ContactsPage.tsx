  ...
  return (
    <section className="page contacts-page">
      ...
      <form className="admin-form contact-manual-search" onSubmit={(event) => { event.preventDefault(); setNotice(""); if (manualQuery.trim().length >= 2) searchMutation.mutate(); }}>
        ...
        <div className="form-grid"><label>Arama türü<select value={manualType} onChange={(event) => setManualType(event.target.value as typeof manualType)}><option value="name">Kullanıcı adı veya Ad Soyad</option><option value="email">E-posta adresi</option><option value="phone">Telefon numarası</option></select></label><label>{manualType === "name" ? "Kullanıcı adı veya Ad Soyad" : manualType === "email" ? "E-posta" : "Telefon"}<input inputMode={manualType === "phone" ? "tel" : "text"} minLength={2} onChange={(event) => setManualQuery(event.target.value)} placeholder={manualType === "name" ? "@kullanici veya Ad Soyad" : manualType === "email" ? "uye@example.com" : "+905551234567"} required type={manualType === "email" ? "email" : "text"} value={manualQuery}/>{manualType === "phone" ? <span className="form-help">Numarayı + ve ülke koduyla yazın (ör. +90 555 123 45 67).</span> : null}</label></div>
