...
export function HelpCenterPage() {
  ...
  if (location.pathname.endsWith("/rules")) {
    ...
    return <div className="page help-center-page"><Link className="back-link" to="/help"><ArrowLeft size={16}/> {copy("Yardım merkezine dön", "Back to Help Centre")}</Link><div className="help-page-heading"><span className="eyebrow">{copy("Topluluk standartları", "Community standards")}</span><h1>{copy("Konnektora Kuralları", "Konnektora Rules")}</h1><p>{copy("İçerik türüne göre aktif kurallar ve ihlal puanları.", "Active rules and violation points by content type.")}</p></div>{rulesQuery.isLoading ? <p role="status">{copy("Kurallar yükleniyor…", "Loading rules…")}</p> : null}{rulesQuery.isError ? <p className="form-error">{copy("Kurallar şu anda yüklenemiyor.", "Rules are currently unavailable.")}</p> : null}<div className="help-rules-groups">{groups.map(([targetType, rules]) => <section className="admin-form" key={targetType}><h2>{(language === "tr" ? { tag: "İlgi alanları", event: "Etkinlikler", place: "Mekânlar", user: "Profiller", username: "Kullanıcı adları", media: "Medya", website_url: "Web siteleri", tag_comment: "İlgi alanı yorumları", event_comment: "Etkinlik yorumları", place_comment: "Mekân yorumları", comment_reply: "Yorum yanıtları", post: "Postlar", post_comment: "Post yorumları", private_message: "Özel mesajlar" } : { tag: "Interests", event: "Events", place: "Places", user: "Profiles", username: "Usernames", media: "Media", website_url: "Websites", tag_comment: "Interest comments", event_comment: "Event comments", place_comment: "Place comments", comment_reply: "Comment replies", post: "Posts", post_comment: "Post comments", private_message: "Private messages" })[targetType as "tag"] ?? targetType}</h2>{rules?.map((rule) => <details className="help-rule-card" key={rule.id}><summary><strong>{rule.title}</strong><span>{rule.violationScore} {copy("ihlal puanı", "violation points")}</span></summary>{rule.description ? <p>{rule.description}</p> : <p>{copy("Açıklama eklenmemiş.", "No description provided.")}</p>}</details>)}</section>)}</div>{!rulesQuery.isLoading && !groups.length ? <div className="help-empty">{copy("Aktif Konnektora kuralı bulunmuyor.", "No active Konnektora rules were found.")}</div> : null}</div>;
  }
