...
export function HelpCenterPage() {
  ...
  if (location.pathname.endsWith("/rules")) {
    ...
    return <div className="page help-center-page"><Link className="back-link" to="/help"><ArrowLeft size={16}/> Yardım merkezine dön</Link><div className="help-page-heading"><span className="eyebrow">Topluluk standartları</span><h1>Konnektora Kuralları</h1><p>İçerik türüne göre aktif kurallar ve ihlal puanları.</p></div>{rulesQuery.isLoading ? <p role="status">Kurallar yükleniyor…</p> : null}{rulesQuery.isError ? <p className="form-error">Kurallar şu anda yüklenemiyor.</p> : null}<div className="help-rules-groups">{groups.map(([targetType, rules]) => <section className="admin-form" key={targetType}><h2>{({ tag: "İlgi alanları", event: "Etkinlikler", place: "Mekânlar", user: "Profiller", comment: "Postlar ve yorumlar", message: "Özel mesajlar", media: "Medya" } as Record<string, string>)[targetType] ?? targetType}</h2>{rules?.map((rule) => <details className="help-rule-card" key={rule.id}><summary><strong>{rule.title}</strong><span>{rule.violationScore} ihlal puanı</span></summary>{rule.description ? <p>{rule.description}</p> : <p>Açıklama eklenmemiş.</p>}</details>)}</section>)}</div>{!rulesQuery.isLoading && !groups.length ? <div className="help-empty">Aktif Konnektora kuralı bulunmuyor.</div> : null}</div>;
  }

  if (faqsQuery.isLoading) return <div className="page help-center-page"><h1>Yardım merkezi</h1><p role="status">Yardım içerikleri yükleniyor…</p></div>;
  if (faqsQuery.isError) return <div className="page help-center-page"><h1>Yardım merkezi</h1><p className="form-error">Yardım içerikleri şu anda yüklenemiyor.</p></div>;
