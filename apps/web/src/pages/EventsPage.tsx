...
export function EventsPage() {
  ...
  const discoveryDefinitions = [
    ...
    { key: "today", title: "Bugünkü etkinlikler", params: { dateFrom: today.toISOString().slice(0, 10), dateTo: tomorrow.toISOString().slice(0, 10) }, auth: false },
    ...
    { key: "popular", title: "Popüler etkinlikler", params: { scope: "popular" }, auth: false },
  ...
  const discoveryQueries = useQueries({ queries: discoveryDefinitions.map((definition) => ({
    ...
    queryFn: () => listEvents(new URLSearchParams({ ...definition.params, pageSize: "8" })),
      ...
      <div className="events-content">
        ...
        {!mapOpen && searchParams.size === 0 ? <div className="event-discovery-sections">
          {discoveryDefinitions.map((definition, index) => {
            ...
            return <section key={definition.key}><header><h2>{definition.title}</h2><span>{discoveryQueries[index]?.data?.total ?? items.length} etkinlik</span></header><div className="event-grid">{items.map((event) => <EventCard event={event} key={event.id}/>)}</div></section>;
          })}
