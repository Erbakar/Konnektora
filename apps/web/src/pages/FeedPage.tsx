...
function ActivityCard({ activity, userId, onChanged }: { activity: DiscoveryFeed["activities"][number]; userId?: string; onChanged: () => void }) {
  ...
  return <article className="post-card feed-activity-card"><header><div className="post-avatar">{activity.kind === "event" ? "E" : activity.kind === "place" ? "M" : activity.kind === "tag" ? "#" : "K"}</div><div className="post-author"><strong>{activity.action}</strong><span>{relativeTime(activity.occurredAt)}</span></div></header>{activity.imageUrl ? <img className="activity-cover" alt="" src={resolveMediaUrl(activity.imageUrl)}/> : null}<h2><Link to={activity.href}>{activity.title}</Link></h2>{activity.subtitle ? <p><RichText text={activity.subtitle}/></p> : null}<small>{activity.meta}</small><div className="post-actions"><Link to={activity.href}>Detayları gör</Link><button onClick={() => void shareActivity(activity)}>Paylaş</button>{mine && (activity.kind === "event" || activity.kind === "place") ? <><button disabled={edit.isPending} onClick={() => { const title = window.prompt(activity.kind === "event" ? "Etkinlik adını düzenle" : "Mekân adını düzenle", activity.title); if (title?.trim() && title.trim() !== activity.title) edit.mutate(title.trim()); }}>Düzenle</button><button disabled={archive.isPending} onClick={() => window.confirm("İçerik yayından kaldırılsın mı?") && archive.mutate()}>Sil</button></> : null}{!mine && activity.ownerId ? <><Link aria-label="Mesaj gönder" title="Mesaj gönder" to={`/messages?peer=${activity.ownerId}`}><Mail size={17}/></Link><button onClick={() => setGuestListOpen(true)}>Guest List'e ekle</button></> : null}{!mine && userId ? <button onClick={() => setReportOpen(true)}>Rapor et</button> : null}</div>{guestListOpen ? <div className="emotion-modal" role="dialog" aria-modal="true"><div><button aria-label="Kapat" onClick={() => setGuestListOpen(false)}>×</button><h2>Guest List'e ekle</h2><div className="admin-list">{managedEvents.data?.map((event) => <button className="admin-list-row" disabled={guestInvite.isPending} key={event.id} onClick={() => guestInvite.mutate(event.id)}><strong>{event.title}</strong></button>)}</div>{!managedEvents.isLoading && !managedEvents.data?.length ? <p className="form-help">Yönettiğin etkinlik bulunmuyor.</p> : null}</div></div> : null}<ReportDialog onClose={() => setReportOpen(false)} open={reportOpen} targetId={activity.id} targetType={activity.kind as ReportTargetType}/></article>;
}

function PostCard({ post, userId, onChanged }: { post: SocialPost; userId?: string; onChanged: () => void }) {
  const client = useQueryClient(); const [open, setOpen] = useState(false); const [comment, setComment] = useState(""); const [guestListOpen, setGuestListOpen] = useState(false); const [reportOpen, setReportOpen] = useState(false);
...
async function sharePost(post: SocialPost) { const url = new URL(window.location.href); url.hash = `post-${post.id}`; const data = { title: `${post.author.name} gönderisi`, text: post.body, url: url.toString() }; if (navigator.share) await navigator.share(data); else { await navigator.clipboard.writeText(`${post.body}\n${url.toString()}`); window.alert("Gönderi bağlantısı kopyalandı."); } }
