...
import { changePassword, deleteProfileMedia, getMyProfile, getNotificationPreferences, getPrivacySettings, getProfileAffinities, getUserSession, listProfileMedia, listTags, makeProfilePicture, resolveMediaUrl, updateMyProfile, updateNotificationPreferences, updatePrivacySettings, updateProfileAffinities, uploadProfileMedia } from "../lib/api";
...
function ProfileSettings() {
  ...
  const tags = useQuery({ queryKey: ["tags", "profile-settings"], queryFn: listTags });
  const affinities = useQuery({ queryKey: ["profile-affinities", "settings"], queryFn: getProfileAffinities });
  const save = useMutation({ mutationFn: updateMyProfile, onSuccess: () => client.invalidateQueries({ queryKey: ["profile"] }) });
  const saveInterests = useMutation({ mutationFn: updateProfileAffinities, onSuccess: () => client.invalidateQueries({ queryKey: ["profile-affinities"] }) });
  if (!profile.data || !tags.data || !affinities.data) return <div className="identity-panel">Profil yükleniyor…</div>;
  const currentSentiments = new Map((affinities.data ?? []).map((item) => [item.tag.id, item.sentiment]));
  return <div className="settings-profile-stack"><form className="identity-panel settings-form" onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = new FormData(event.currentTarget); const value = (key: string) => String(form.get(key) || "").trim() || undefined; save.mutate({ name: value("name")!, username: value("username"), phone: value("phone"), city: value("city"), country: value("country"), district: value("district"), address: value("address"), website: value("website") }); }}><div className="form-grid"><label>Ad soyad<input defaultValue={profile.data.name} name="name" required/></label><label>Kullanıcı adı<input defaultValue={profile.data.username ?? ""} name="username"/></label><label>Telefon<input defaultValue={profile.data.phone ?? ""} name="phone"/></label><label>Şehir<input defaultValue={profile.data.city ?? ""} name="city"/></label><label>Ülke<input defaultValue={profile.data.country ?? ""} name="country"/></label><label>İlçe<input defaultValue={profile.data.district ?? ""} name="district"/></label><label>Adres<input defaultValue={profile.data.address ?? ""} name="address"/></label><label>Web sitesi<input defaultValue={profile.data.website ?? ""} name="website" type="url"/></label></div><button className="primary-action" disabled={save.isPending}>Kaydet</button>{save.isSuccess ? <p className="form-success">Profil kaydedildi.</p> : null}{save.isError ? <p className="form-error">Profil kaydedilemedi.</p> : null}</form><form className="identity-panel settings-form" onSubmit={(event: FormEvent<HTMLFormElement>) => { event.preventDefault(); const form = new FormData(event.currentTarget); saveInterests.mutate(form.getAll("tagIds").map(String).map((tagId) => ({ tagId, sentiment: String(form.get(`sentiment:${tagId}`) || currentSentiments.get(tagId) || "like") as "like" | "ok" | "dislike" }))); }}><h2>İlgi alanları</h2><p>Profilinde göstermek istediğin etiketleri ve duygunu seç.</p><div className="settings-interest-grid">{tags.data?.map((tag) => <article key={tag.id}><label><input defaultChecked={currentSentiments.has(tag.id)} name="tagIds" type="checkbox" value={tag.id}/><span>#{tag.name}</span></label><select aria-label={`${tag.name} duygusu`} defaultValue={currentSentiments.get(tag.id) ?? "like"} name={`sentiment:${tag.id}`}><option value="like">Beğeniyorum</option><option value="ok">Nötr</option><option value="dislike">Beğenmiyorum</option></select></article>)}</div><button className="primary-action" disabled={saveInterests.isPending}>İlgi alanlarını kaydet</button>{saveInterests.isSuccess ? <p className="form-success">İlgi alanları güncellendi.</p> : null}{saveInterests.isError ? <p className="form-error">İlgi alanları güncellenemedi.</p> : null}</form></div>;
}
